...
export default class AppProcessor {
	...
	async buildModelQueryObject(pagination, queryParser = null) {
		...
		// let query = JSON.parse(await AppStorage.existInStorage(modelName));
		// if(_.isEmpty(query)){
			
		// 	console.log(query.tree);
		// 	await AppStorage.saveToStorage(modelName, JSON.stringify([...query.exec()]));
		// }
		...
		// const value = await query.select(queryParser.selection).exec();
		// console.log(value);
		return {
	...
	async buildModelAggregateQueryObject(pagination, query, queryParser = null) {
		...
		query.push({
			$sort: (queryParser.sort) ?
				Object.assign({}, {...queryParser.sort, createdAt: -1 }) : { createdAt: -1 }
		});
		if (!queryParser.getAll) {
			query.push({
